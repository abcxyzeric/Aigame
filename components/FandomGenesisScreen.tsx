...
const FandomGenesisScreen: React.FC<FandomGenesisScreenProps> = ({ onBack }) => {
  ...
  const handleGenerateSummary = async () => {
    ...
    try {
      ...
      const fileName = `${workName.replace(/[\s/\\?%*:|"<>]/g, '_')}_summary.txt`;
  ...
  const handleGenerateArcSummaries = async () => {
    ...
    try {
        ...
        for (let i = 0; i < arcList.length; i++) {
            ...
            const jsonContent = await aiService.generateFandomGenesis(selectedSummary.content, arcName, workName, authorName);
            const fileName = `${workName.replace(/[\s/\\?%*:|"<>]/g, '_')}_${arcName.replace(/[\s/\\?%*:|"<>]/g, '_')}.json`;
            fandomFileService.saveFandomFile(fileName, jsonContent);
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6 md:p-8">
        ...
        <div className="space-y-8">
            ...
            <div className="bg-slate-800/60 backdrop-blur-sm rounded-lg p-6 border border-slate-700/50">
                ...
                <p className="text-slate-400 mb-4 text-sm">Chọn tệp tóm tắt tổng quan (.txt) từ kho. AI sẽ tự động phân tích và tóm tắt chi tiết về nhân vật, địa điểm, sự kiện... trong từng Arc/Saga, sau đó xuất ra các tệp .json riêng biệt và lưu vào kho.</p>
